...
const List = () => {
  ...
  {
    /*const determineShowList = (selectedItem, selectedId) => {
    if (selectedItem === selectedId) {
      ...
      showList && (
        <View className="flex-row justify-evenly">
          ...
          />
          <TouchableOpacity
            onPress={() => increaseQuantity(num, book.quantity, book.id)}
            className={
          ...
          <TouchableOpacity
            onPress={() => decreaseQuantity(num, book.quantity, book.id)}
            className={
              "bg-black-100 rounded-xl h-15 w-28 justify-center items-center "
            }
          >
            <Text className="text-white font-psemibold text-xs">Remove</Text>
          </TouchableOpacity>
        </View>
      );
    }
          ...
          {books.map((book, index) => {
            return (
              <TouchableOpacity key={index}>
                <Books
                  key={index}
                  handlePress={() => setShowList(!showList)}
